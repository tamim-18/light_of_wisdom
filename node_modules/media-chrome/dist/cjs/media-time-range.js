...
template.innerHTML = `
  <style>
    ...
    media-preview-chapter-display[${import_constants.MediaUIAttributes.MEDIA_PREVIEW_CHAPTER}]:not([${import_constants.MediaUIAttributes.MEDIA_PREVIEW_CHAPTER}=""]),
    ::slotted(media-preview-chapter-display[${import_constants.MediaUIAttributes.MEDIA_PREVIEW_CHAPTER}]:not([${import_constants.MediaUIAttributes.MEDIA_PREVIEW_CHAPTER}=""])) {
      display: initial;
    }
...
class MediaTimeRange extends import_media_chrome_range.MediaChromeRange {
  ...
  updateBufferedBar() {
    ...
    const { style } = (0, import_element_utils2.getOrInsertCSSRule)(this.shadowRoot, "#highlight");
  ...
  updateCurrentBox() {
    ...
    const { style } = (0, import_element_utils2.getOrInsertCSSRule)(this.shadowRoot, "#current-rail");
...
shouldRangeAnimate_fn = function() {
  return this.isConnected && !this.mediaPaused && !this.mediaLoading && !this.mediaEnded && this.mediaDuration > 0 && (0, import_element_utils.isElementVisible)(this);
};
...
handlePointerMove_fn = function(evt) {
  ...
  const { style } = (0, import_element_utils2.getOrInsertCSSRule)(this.shadowRoot, "#preview-rail");
  style.transform = `translateX(${boxPos})`;
...
previewRequest_fn = function(detail) {
  this.dispatchEvent(new import_server_safe_globals.globalThis.CustomEvent(
...
seekRequest_fn = function() {
  ...
  this.dispatchEvent(new import_server_safe_globals.globalThis.CustomEvent(
    import_constants.MediaUIEvents.MEDIA_SEEK_REQUEST,
    { composed: true, bubbles: true, detail }
  ));
};
